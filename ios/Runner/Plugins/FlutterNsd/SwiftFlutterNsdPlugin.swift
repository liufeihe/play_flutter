...
public class SwiftFlutterNsdPlugin: NSObject, FlutterPlugin, NetServiceBrowserDelegate, NetServiceDelegate {
    ...
    public static func register(with registrar: FlutterPluginRegistrar) {
        let flutterNsdChannel = FlutterMethodChannel(name: "flutter_nsd", binaryMessenger: registrar.messenger())
    ...
    public func netServiceDidResolveAddress(_ sender: NetService) {
        channel.invokeMethod(
                "onServiceResolved",
                arguments: ["hostname": sender.hostName, "port": sender.port, "name": sender.name, "addresses": sender.addresses])
    }
}
